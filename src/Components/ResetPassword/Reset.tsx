...
import { IMessageDetail } from "../../Interfaces/interface"
...
const ResetPassword = () => {
    ...
    const resendCode = async () => {
        try {
            const res = await axiosInst.get('/api/Auth/ResendCode', { params: { email, state: "forgot" } });
            if (res) {
    ...
    const sendData = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        try {
            const res = await axiosInst.post("/api/Auth/resetPassword", { email, newPassword, otp })
                ...
                <div className="fixed inset-0 bg-gray-900/75 h-full z-50 flex items-center justify-center" onClick={() => setIsSuccess(false)}>
                    <div className="bg-white p-6 rounded-md shadow-md max-w-md w-full text-center animate__animated animate__fadeInDown" onClick={(e) => { e.stopPropagation() }}>
                        <div className="flex flex-col items-center gap-1">
                            ...
                            <p className="text-lg font-semibold">Your password reset successfully</p>
                            <p className="text-sm">now returning to login page ...</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="mt-3 p-4 bg-red-50 rounded-lg border text-start border-red-100">
                        ...
                        <div className="flex flex-col te gap-1">
                            {errorMessage.map((msg: IMessageDetail, index: number) => (
                                ...
                                <p className="text-red-500 text-xs" key={index}>* {msg.message}</p></div>
                            ))}
                ...
                <form className="space-y-6" onSubmit={sendData}>
                  ...
                  <AtSign className="h-5 w-5 text-gray-400" />
                            ...
                            <Input
                                ...
                                onChange={handleData}
                                value={newPassword}
                                className="block w-full pl-5 pr-3 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white placeholder-gray-400"
                                placeholderType="Enter your new password"
                                required />
                        </div>
